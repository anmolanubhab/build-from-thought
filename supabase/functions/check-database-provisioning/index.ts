...
async function finishProvisioning(opts: {
  ...
  if (!rawSchema) {
    const message = "This project has no supabase/schema.sql to provision.";
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      return json({ error: finishErr instanceof Error ? finishErr.message : "Failed to finish provisioning" }, 502);
    }
  } catch (e) {
